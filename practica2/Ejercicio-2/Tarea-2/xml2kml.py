...
def fileSeparator(miArchivoTXT, miArchivoKML):
    f = open(miArchivoTXT, "r", encoding='utf-8')
    line = f.read(-1)
    lines = line.split("SPLITTOKEN")
    for x in range (3):
        outfile = miArchivoKML + (str)(x) + ".kml"
...
def main():
    ...
    miArchivoTXT = input('Introduzca un nombre para el archivo TXT intermedio = ')
    miArchivoKML = input('Introduzca un nombre para los archivos KML = ')
    #miArchivoXML = "rutas.xml"
    #miArchivoTXT = "ruta.txt"
    writeXML(miArchivoXML, miArchivoTXT)
    fileSeparator(miArchivoTXT, miArchivoKML)
